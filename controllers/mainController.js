      ...
      if (error) {
        ...
        result.forEach(function(item) {
          ...
          if (existing.length) {
            ...
            output[existingIndex].attraction = output[
              existingIndex
            ].attraction.concat(item.attraction);
            output[existingIndex].time = output[existingIndex].time.concat(
              item.time
            );
          } else {
            if (
  ...
  let getATrip = (request, response) => {
    ...
    db.trips.getAUserTrip(userId, tripId, (error, result) => {
      if (error) {
        ...
        db.trips.getNoDataTrip(userId, tripId, (error, otherResult) => {
          const data = {
            message: "NO ITIN",
